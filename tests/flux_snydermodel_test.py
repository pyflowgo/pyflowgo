...
class MyTestCase(unittest.TestCase):
    ...
    def test_read_initial_condition_from_json_file(self):
        ...
        flowgo_flux_radiation_heat = pyflowgo.flowgo_flux_snyder2002fig4_heat.FlowGoFluxSnyderHeat(terrain_condition,
                                                                                                   material_lava,
        ...
        self.assertEqual(flowgo_flux_radiation_heat._air_temperature, 293.15)

    def test_compute_effective_temperature_snyder(self):
        filename = './resources/input_parameters_radiation_flux.json'
        terrain_condition = pyflowgo.flowgo_terrain_condition.FlowGoTerrainCondition()
        ...
        effective_temperature_snyder = flowgo_flux_radiation_heat.compute_effective_temperature_snyder(state, terrain_condition)

        self.assertAlmostEqual(effective_temperature_snyder, 788.086249, 5)
    ...
    def test_compute_flux(self):
        ...
        flowgo_flux_qsnyder_heat = pyflowgo.flowgo_flux_snyder2002fig4_heat.FlowGoFluxSnyderHeat(terrain_condition,
                                                                                                 material_lava,
                                                                                                 crust_temperature_model_constant,
                                                                                                 effective_cover_crust_model_basic)
        flowgo_flux_qsnyder_heat.read_initial_condition_from_json_file(filename)

        effective_radiation_temperature = 0
        print('effective_radiation_temperature=',effective_radiation_temperature)

        qsnyder = flowgo_flux_qsnyder_heat.compute_flux(state,channel_depth=1.4,
                                                                      channel_width=6.20861841803369)
        # if T air not considered :
        self.assertAlmostEqual(qsnyder, 74261.109313, 4)
        #self.assertAlmostEqual(qsnyder, 60275.150121, 4)
