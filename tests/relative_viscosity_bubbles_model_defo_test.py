...
class MyTestCase(unittest.TestCase):
    ...
    def test_compute_relative_viscosity_bubbles(self):
        ...
        relative_viscosity_bubbles_model_defo = pyflowgo.flowgo_relative_viscosity_bubbles_model_defo. \
            FlowGoRelativeViscosityBubblesModelDefo(vesicle_fraction_model=vesicle_fraction_model_constant)

        state = pyflowgo.flowgo_state.FlowGoState()
        state.set_crystal_fraction(0.2)

        relative_viscosity_bubbles = relative_viscosity_bubbles_model_defo.compute_relative_viscosity_bubbles(state)
        self.assertAlmostEqual(relative_viscosity_bubbles, 0.1821814557051,10)
