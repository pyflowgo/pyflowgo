...
class MyTestCase(unittest.TestCase):
    ...
    def test_compute_relative_viscosity(self):
        ...
        relative_viscosity_model_ptp2 = pyflowgo.flowgo_relative_viscosity_model_ptp2. \
            FlowGoRelativeViscosityModelPhanThienPham2(vesicle_fraction_model=vesicle_fraction_model_constant)

        state = pyflowgo.flowgo_state.FlowGoState()
        state.set_crystal_fraction(0.2)

        relative_viscosity = relative_viscosity_model_ptp2.compute_relative_viscosity(state)
        self.assertAlmostEqual(relative_viscosity, 12.0260465532,10)
