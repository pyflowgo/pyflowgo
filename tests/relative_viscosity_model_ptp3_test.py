...
class MyTestCase(unittest.TestCase):
    ...
    def test_compute_relative_viscosity(self):
        ...
        relative_viscosity_model_ptp3 = pyflowgo.flowgo_relative_viscosity_model_ptp3. \
            FlowGoRelativeViscosityModelPhanThienPham3(vesicle_fraction_model=vesicle_fraction_model_constant)

        state = pyflowgo.flowgo_state.FlowGoState()
        state.set_crystal_fraction(0.2)

        relative_viscosity = relative_viscosity_model_ptp3.compute_relative_viscosity(state)
        self.assertAlmostEqual(relative_viscosity, 8.73464053710,10)
