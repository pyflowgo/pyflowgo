...
""" This is a model from Biren et al 2023 where radiative heat flux is calculated from a temperature-dependent 
emissivity :

epsilon_effective_SWIR = 0.586+7.123 10^(-4) * effective_radiation_temperature - 3.084 10^(-7) * effective_radiation_temperature ** 2 -BIREN
epsilon_effective_TIR = 0.826-8.235 10^(-5) * effective_radiation_temperature + 7.818 10^(-8)   -BIREN
References:
...
class FlowGoFluxRadiationHeat(pyflowgo.base.flowgo_base_flux.FlowGoBaseFlux):
    ...
    def _compute_emissivity_crust(self, state, terrain_condition):
        crust_temperature = self._crust_temperature_model.compute_crust_temperature(state)
        emissivity_crust = 0.586 + 7.123e-4 * crust_temperature - 3.084e-7 * crust_temperature ** 2
        return emissivity_crust

    def _compute_emissivity_molten(self, state, terrain_condition):
        molten_material_temperature = self._material_lava.computes_molten_material_temperature(state)
        emissivity_molten = 0.586 + 7.123e-4 * molten_material_temperature - 3.084e-7 * molten_material_temperature ** 2
    ...
    def _compute_spectral_radiance(self, state, terrain_condition, channel_width):
        ...
        # Constants
        lamda = 1.6e-6   # micrometers Biren # micrometers ALI unsaturated band 7 data, band center at: 0.8675e-6
